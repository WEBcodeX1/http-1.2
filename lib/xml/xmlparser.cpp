...
template <typename Callback>
void forEachMessageSlice(std::string_view Input, bool& FramingError, Callback&& Visit)
{
    std::size_t Cursor = 0;
    ...
    while (true) {
        ...
        if (!Visit(MessageSlice{Input.substr(Start, MessageEnd - Start)})) {
            return;
        }
...
ParseResult_t XMLParser::parse(char* InputBuffer) const
{
    ...
    forEachMessageSlice(InputBufferSV, FramingError, [&](const MessageSlice& Slice) {
        ResultTree_t Tree;
